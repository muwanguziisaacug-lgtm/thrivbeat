...
export default function LeadForm({ kind = "GENERAL" }) {
  ...
  return (
    <form onSubmit={submit} className="rounded-3xl border border-stone-200 bg-white p-6 shadow-sm sm:p-8" noValidate>
      ...
      {kind === "ASSESSMENT" && <div className="mt-6"><fieldset><legend className="font-semibold text-stone-800">What would you like support management with? <span aria-hidden="true">*</span></legend><div className="mt-3 grid gap-2 sm:grid-cols-2">{["High blood pressure,", "Diabetes", "Obesity", "Arthritis", "Osteoporosis"].map((goal) => <label key={goal} className="flex gap-3 rounded-xl bg-stone-50 p-3"><input type="checkbox" name="goals" value={goal} />{goal}</label>)}</div></fieldset><fieldset className="mt-6"><legend className="font-semibold text-stone-800">Goals for heart health and mobility</legend><div className="mt-3 grid gap-2 sm:grid-cols-2">{["Move with more confidence", "Build balance", "Build flexibility", "Cardiovascular fitness", "Return to walking"].map((goal) => <label key={goal} className="flex gap-3 rounded-xl bg-stone-50 p-3"><input type="checkbox" name="goals" value={goal} />{goal}</label>)}</div></fieldset><label className="mt-5 block font-semibold text-stone-800">Broad mobility-support needs<select className={inputClass} name="mobilitySupport" required defaultValue=""><option value="" disabled>Select one</option><option value="NONE">No support at present</option><option value="SOME">Some support</option><option value="REGULAR">Regular support</option><option value="UNSURE">Not sure</option></select></label></div>}
